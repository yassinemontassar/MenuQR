...
const UnsplashDialog: React.FC<UnsplashDialogProps> = ({ onImageSelect }) => {
  ...
  // Fetch photos based on search term using useEffect
  const fetchPhotos = useCallback(async () => {
    setIsLoading(true); // Set loading to true before the API call
    try {
      if (searchTerm) {
        const response = await axios.get(`/api/unsplash/?query=${searchTerm}`);
  ...
  // Debounce the fetchPhotos function
  const debouncedFetchPhotos = useCallback(
    debounce(fetchPhotos, 1200),
    [fetchPhotos]
  );
  ...
  useEffect(() => {
    ...
    return () => debouncedFetchPhotos.cancel();
  }, [debouncedFetchPhotos]);
  return (
